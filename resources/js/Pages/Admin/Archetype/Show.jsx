...
function Show({ archetype, errors }) {
	const [values, setValues] = useState({
		id: archetype.id,
		archetype_code: archetype.archetype_code,
		name_ja: archetype.name_ja,
	...
	const handleSubmit = e => {
		e.preventDefault();
		router.put(`/admin/archetype/${values.id}`, values);
	};

	// archetypeの削除
	const handleDelete = e => {
		e.preventDefault();
		router.delete(`/admin/archetype/${values.id}`);
	};
		...
		<>
			<div className="w-2/3 pt-8 mx-auto">
				<div className="flex justify-between">
					<h1 className="font-bold text-3xl mb-8">archetype詳細</h1>
					<Link href={route('admin.archetype.index')} className="hover:text-blue-400">
						{'< '} archetypes一覧へ戻る
					</Link>
				</div>
				<h2 className="text-lg">archetype詳細を確認・編集</h2>
				<div className="p-8 mb-4 bg-gray-100 rounded-md">
					...
					<form onSubmit={handleSubmit} method="POST">
						<table className="show-table">
							...
							<tbody>
								<tr>
									<th className="text-right">archetype_code:　</th>
									<td>
										<input
											name="archetype_code"
											type="text"
				...
				<h2 className="text-lg">archetypeを削除</h2>
...
Show.layout = page => <AdminLayout title="archetypes詳細" children={page} />;
