...
function getStyles(frameTypeVal, frameTypeVals, theme) {
	return {
		fontWeight:
			frameTypeVals.indexOf(frameTypeVal) === -1
...
function FrameTypeFilterMUI({ filters }) {
  ...
  const [frameTypeArray, setFrameTypeArray] = React.useState([]);
	const [frameTypeVals, setFrameTypeVals] = React.useState([]);
          ...
          autoWidth
					...
					{frameTypeArray.map(frameType => (
						<MenuItem
							...
							disableRipple
							style={getStyles(frameType.name_en, frameTypeVals, theme)}>
							<Checkbox checked={frameTypeVals.indexOf(frameType.name_en) > -1} />
