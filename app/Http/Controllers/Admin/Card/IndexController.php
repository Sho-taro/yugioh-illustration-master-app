    ...
    // 登録されているカードのレコード数を取得
      ...
      if ($target === 'monster') {
        ...
        if (!is_null($request->input('frame-types'))) {
          $cards_query->whereIn('frame_types.name_en', $request->input('frame-types'));
        }

        // raceの条件で絞り込みするクエリを生成
        if (!is_null($request->input('races'))) {
          $cards_query->whereIn('races.name_en', $request->input('races'));
        }

        // attributeの条件で絞り込みするクエリを生成
        if (!is_null($request->input('attributes'))) {
          $cards_query->whereIn('attributes.name_en', $request->input('attributes'));
        }

        // level_or_rankの条件で絞り込みするクエリを生成
        if (!is_null($request->input('level-or-ranks'))) {
          $cards_query->whereIn('monster_card_details.level_or_rank', $request->input('level-or-ranks'));
        }

        // link_valueの条件で絞り込みするクエリを生成
        if (!is_null($request->input('link-values'))) {
          $cards_query->whereIn('monster_card_details.link_value', $request->input('link-values'));
        }
        ...
        $cards_query->where('frame_types.name_en', '=', 'spell');

        // play-typeの条件で絞り込みするクエリを生成
        if (!is_null($request->input('play-types'))) {
          $cards_query->whereIn('spell_trap_play_types.name_en', $request->input('play-types'));
        }
      } else if ($target === 'trap') {
        // 絞り込み対象がspellの場合
        $cards_query
        ...
        $cards_query->where('frame_types.name_en', '=', 'trap');
      ...
      // dd($cards_query->count());
      $message = "{$cards_query->count()} 枚のカードが見つかりました。";

      // クエリを実行してレコードを取得
      $data = $cards_query->orderBy('name_ja_kana', 'ASC')->paginate(15)         // 日本語カード名（読み）の昇順
